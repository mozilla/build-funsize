...
def build_partial_mar(to_mar_url, to_mar_hash, from_mar_url, from_mar_hash,
    ...
    """
    logging.debug('Creating temporary working directories')
    ...
    try:
        ...
        logging.debug('Partial MAR generated at %s', partial_file)
    except:
        cacheo.delete_from_cache(identifier, 'partial')
        raise

    logging.info('Saving partial MAR %s to cache with key %s',
                 partial_file, identifier)
...
def generate_partial_mar(to_mar, from_mar, channel_id, product_version,
    ...
    logging.info('Unwrapping "to" MAR')
    ...
    logging.info('Unwrapping "from" MAR')
    ...
    logging.info('Generating partial mar @ %s', partial_mar)
    out = sh.bash(MAKE_INCREMENTAL, partial_mar, from_mar_wd, to_mar_wd,
                  _cwd=working_dir, _env=my_env, _timeout=300, _err_to_out=True)
    logging.debug("Command returned: %s", out)
    logging.info('Partial now available at path: %s', partial_mar)
