...
def extract_mar(filename, work_dir): 
    ...
    try:
        os.chdir(work_dir)
        #exec_shell_cmd("mar -x "+filename) TODO: Fix install assumption
        exec_shell_cmd("/tmp/tools/mar -x "+filename)
    finally:
        os.chdir(saved_path)
...
def create_partial_patch_for_file(from_marfile_entry, to_marfile_entry, shas, patch_info):
    ...
    if not (from_marfile_entry.sha(),to_marfile_entry.sha()) in shas:
        ...
        #exec_shell_cmd("mbsdiff "+from_marfile_entry.abs_path+" "+to_marfile_entry.abs_path+" "+patch_file_abs_path) TODO: Fix install assumption
        exec_shell_cmd("/tmp/tools/mbsdiff "+from_marfile_entry.abs_path+" "+to_marfile_entry.abs_path+" "+patch_file_abs_path)
...
def create_partial_patch(from_dir_path, to_dir_path, patch_filename, shas, patch_info, forced_updates, add_if_not_list):
    ...
    print "Inside create_partial_patch"
    print "SHAs: ",shas
    ...
    #mar_cmd = 'mar -C '+patch_info.work_dir+' -c output.mar '+string.join(patch_info.archive_files, ' ') TODO: Fix install assumption
    mar_cmd = '/tmp/tools/mar -C '+patch_info.work_dir+' -c output.mar '+string.join(patch_info.archive_files, ' ')
...
def create_partial_patches(patches):
    ...
    try:
        ...
        #for patch_num, patch in enumerate(patches): # Is this better?
            startTime = time.time()
